...
//  game.Light(0, { glm::vec4(200, 500, 50, 1), glm::vec4(400000, 400000, 400000, 1) } );
  ...
  if (game.UserVariable(Global::varPlayerStock) >= 0) {
    ...
    if (poppingTimer <= 0) {
      ...
      for (int i = rndPoppingCount(game.Rand()); i > 0; --i) {
        ...
        if (Entity::Entity* p = game.AddEntity(Global::EntityGroupId_Enemy, pos, "Toroid", "Res/Model/Toroid.bmp", "Res/Model/Toroid.Normal.bmp", UpdateToroid(pPlayer, enemyLevel))) {
...
//  snprintf(str, 16, "%03.0f", game.Fps());
  snprintf(str, 16, "%03.2f", game.luminanceScale);
